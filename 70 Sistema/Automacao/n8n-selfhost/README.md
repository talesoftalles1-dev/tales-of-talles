...
**Verificação feita (2026-06-29):** a lógica dos 4 coaches + nó de visão foi executada em Node contra o payload real do PWA → **5/5 OK** (ex.: Muzy=`RECUPERAR`@38%, Cariani=`CUIDADO` PEITO [...])

> ⚠️ **Esta máquina não corre o n8n por npm.** O `npm install -g n8n` falhou a compilar o módulo nativo `isolated-vm` (sandbox do nó Code) — não há Visual Studio Build Tools e o Node v[...]
...
**A2. VM grátis sempre-ligada (Oracle Cloud "Always Free" — sem cobrança):** cria a VM Ubuntu, instala Docker, copia esta pasta, define no `.env` o `WEBHOOK_URL=https://<dominio-ou-ip>/` e `N8[...]
...
```
Copia o `https://<algo>.trycloudflare.com`, põe-no no `.env` como `WEBHOOK_URL=https://<algo>.trycloudflare.com/`, **reinicia** o n8n (regista os webhooks nesse URL). Senão: o URL muda a cada re[...]

---

## Credenciais (uma vez, no self-host)

- **Anthropic** → *Credentials → New → "Header Auth"* · **Name: `Anthropic x-api-key`** · Header Name: `x-api-key` · Value: `sk-ant-...`. Os 3 workflows de visão já apontam para este no[...]
- **Notion** → *New → "Notion API"* · **Name: `Notion API`** · Internal Integration Token; partilha a base de dados do Diário com a integração.

## Apontar o PWA

App → card **🔗 Ligações · n8n** → cola o URL (do túnel/host) em *ENDEREÇO BASE DO n8n* → **Guardar** (fica em `localStorage`, sem redeploy). Para default permanente: troca `N8N_DEFAU[...]
...
Os 3 webhooks de visão são abertos e gastam créditos Anthropic se descobertos. Protege com *Header Auth* (segredo partilhado) no nó Webhook e envia o header a partir do PWA. CORS: podes aperta[...]
